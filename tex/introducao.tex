...
trabalho. O estudo utilizou uma linha de produção simulada em que um grupo de
três pessoas deveriam executar uma série de atividades para construir um objeto
a partir de partes mais simples. Quanto à alocação de recursos às tarefas, os
experimentos foram divididos em três tipo: manual, semi autônoma (na qual um
computador realiza o planejamento e um humano revisa o plano antes da execução)
e totalmente autônoma. Os resultados, apresentados na Figura~\ref{graficoMIT},
mostram que a automação da distribuição do trabalho trouxe ganhos de eficiência
operacional e, mais acentuadamente, gerencial, resultante da tomada de decisões
melhores e em menor tempo.

O impacto econômico da adoção de tecnologia de auxílio ao planejamento de
alocação de recursos pode ser estimado tendo em vista que, em média, 54 por
cento do tempo produtivo de gerentes de todos os níveis é gasto em atividades
de coordenação e controle de recursos~\cite{accenture}, e que ferramentas de
automação poderiam diminuir essa proporção para 25 por cento~\cite{accenture},
liberando aproximadamente 50 homens-hora por mês de cada gerente, além de
possíveis ganhos operacionais derivados da melhor qualidade e maior
...
\begin{figure}[h]
	...
	\caption{Resultados da inserção de elementos automáticos na gerência. Adaptado de \citeonline{gombolay2015decision}.\label{graficoMIT}}
\end{figure}
...
utilizando-se técnicas inteligência artificial, como descrito em
\citeonline{russell1995modern}.

Será utilizado um modelo preditivo da performance da operação, obtido a partir
da aplicação de métodos de aprendizagem de máquina nas informações geradas na
execução do processo operacional para estimar os efeitos futuros das decisões
tomadas pelo sistema. Utilizando-se então os resultados do modelo preditivo
como heurística, será aplicado um algoritmo de busca para realizar o
planejamento.
...
Os sete pontos de vista propostos em \citeonline{rozanski2011software} e
ilustrados na Figura~\ref{rozanski} são associados aos elementos do modelo de
\citeonline{hevner2007three} da seguinte forma:

\begin{figure}[h]
	\centering
	\includegraphics[height=10cm]{visoesrozanski}
...
Esta monografia está organizada em cinco capítulos. O primeiro apresentou o
...
O quarto capítulo consiste na aplicação do software construído em dados do
processo de manutenção preventiva de uma grande empresa brasileira da área de
manufatura industrial de bens intermediários.
