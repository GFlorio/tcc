...
variam com o tempo. Planos a longo prazo utilizam indicadores agregados de
natureza tipicamente financeira, dado que a incerteza da demanda é grande
demais para realizar previsões detalhadas dos acontecimentos. A curto e médio
...
gerados por um outro sistema de informação corporativo~\cite{van2016process}.
Este modelo pode, então, ser utilizado para responder questões a respeito da
estrutura, da performance e da conformidade do processo a normas estabelecidas.
A abordagem de construção automática a partir de dados tem o benefício de gerar
um modelo que é, em geral, mais aderente à realidade do que o gerado
manualmente. Uma vez que o modelo formal é gerado, podem ser criadas
representações dele em diferentes níveis de abstração e detalhe, para
comunicação aos \textit{stakeholders}, sem perder essa propriedade.
...
\subsection{Modelos Preditivos de Performance de Processo}
...
\begin{enumerate}
    \item Associar a cada estado do processo definido por um prefixo de um traço completo um estimador do valor mais provável que $m$ assumirá quando a instância estiver completa.
    \item Estender o traço, criando eventos simulados de forma a completar a instância, calculando o indicador sobre o traço completo.
\end{enumerate}
